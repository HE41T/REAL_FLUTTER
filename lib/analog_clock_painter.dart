...
class AnalogClockPainter extends CustomPainter {
  ...
  _paintPinHole(Canvas canvas, Size size, double scaleFactor) {
  ...
  void _paintTickMarks(Canvas canvas, Size size, double scaleFactor) {
    ...
    for (int i = 1; i <= 60; i++) {
      double len = tick;
      if (i % 15 == 0) {
        len = longTick; // Longest tick on quarters (every 15 ticks)
      } else if (i % 5 == 0) {
        len = mediumTick; // Medium ticks on the '5's (every 5 ticks)
      }
  ...
  void _paintDigitalClock(
    ...
    TextSpan digitalClockSpan = TextSpan(
        ...
        text: "$hour:$minute:$second$meridiem"); // TextSpan
